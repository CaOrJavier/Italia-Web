// Las dos rutas, día a día.
//
// Dos modos: una ruta a lo largo (el de siempre, y el único que cabe en el móvil)
// y las dos en paralelo, alineadas día a día, que es donde se ve de un vistazo
...
// Con dos columnas basta bastante menos sitio que cuando eran seis.
const ANCHA = '(min-width: 820px)';
...
function paralelo(main, params, modoExplicito) {
  ...
  main.innerHTML = `
    <p class="intro">Las dos a la vez, alineadas por día. Pincha en cualquier casilla y se
    abre la fila entera: así ves qué hace la otra ruta ese mismo día.</p>
    ${cambiaModo('tres')}
    ${leyenda()}
    ${comun()}
    <div class="par">
  ...
  return `<div class="par-fila${abierta ? ' abierta' : ''}" data-n="${n}">
    <div class="par-dia">
      <button type="button" class="par-abre" aria-expanded="${abierta}"
              aria-label="Abrir el día ${n} en las dos rutas">
...
function cambiaModo(activo) {
  return `<div class="filtros modo" role="group" aria-label="Cómo ver las rutas">
    ...
    <button type="button" data-modo="tres" aria-pressed="${activo === 'tres'}">Las dos en paralelo</button>
  </div>`;
}
...
function comun() {
  ...
  return `<details class="tarjeta"><summary class="cab-tarjeta" style="cursor:pointer">
      <h3>${esc(c.titulo)}</h3><span class="etiq etiq-gris">Igual en las dos</span>
    </summary>
